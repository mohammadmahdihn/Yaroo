...
def utility_vec(v: Vehicle, f_vec_GHz: float) -> float:
    ...
    # paper says e_vec(max) depends on transmission rate; not fully specified.
    # Use a budget factor around actual offload energy so the constraint matters.
    e_vec_max = p.offload_energy_budget_factor * v.vec_energy_J()
...
def _f_opt_for_lambda(v: Vehicle, lam: float) -> float:
    ...
    # Define:
    # slack(f) = X_vec - K_vec = A - B/f
    # A = wE*(e_vec_max - e_vec) + wT*(t_ptd - alpha/gamma)
    # B = wT*C
    e_vec = v.vec_energy_J()
    e_vec_max = p.offload_energy_budget_factor * e_vec
    t_ptd = v.practical_tolerable_delay()
    tx_time = v.task.alpha_bits / v.rate_bps

    A = p.wE * (e_vec_max - e_vec) + p.wT * (t_ptd - tx_time)
    B = p.wT * v.task.C_Gcycles
    ...
    # KKT leads to:
    # f = ( B + sqrt(B^2 + 4*(1+A)*theta*B / ((1-theta)*rho + lam)) ) / (2*(1+A))
    denom = (1.0 - p.theta) * p.rho_vec + lam
    if denom <= 0.0:
        denom = 1e-12

    inside = B * B + 4.0 * (1.0 + A) * p.theta * B / denom
    f = (B + math.sqrt(max(inside, 0.0))) / (2.0 * (1.0 + A))

    # Ensure f yields positive slack (otherwise utility is just cost, so clamp to 0)
    if A - B / max(f, 1e-12) <= 0.0:
        return 0.0
